...
> - **The sfera turns islands on.** Two half-spheres joined lock forever and switch on island scoring for the rest of the game: at the end of every round, the majority on any piece of country that has broken off the mainland takes a point per tile. It resolves last in the round, which is the whole tactic — blow a rival's figure off the rock *before* the count.
>
...
>
> **Island scoring needed a brake, and finding it is the useful part.** The rule as designed — most figures on an island, 1 per tile, every round — measured at 0 to 492 island points a game across twenty games. Whoever's follower happened to be standing on the big piece when it broke off collected its size thirty times over and nothing else in the game mattered. The fix is one line: a tile pays a given player *once*. A rival who seizes the majority still collects the whole island fresh, and an island that grows still pays its new stones, so every incentive the rule was for survives — but the recurring annuity doesn't. Island income went from 0–492 to a mean of 24.
>
> **Where it landed.** Twenty bot games, per game: island 24.4, temple 23.1, cities 12.8, roads 1.5, skyholds 1.5 — 63 points across two players, scores 4–64, and no single stream over 40%. Closures are up from 0.65 to 2.35 a game. A sharp bot beats random play 34–5 in forty games, which says the three economies are actually readable rather than noise.
